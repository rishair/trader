...
function generateApprovalId(existingIds: string[]): string {
  // Short, memorable IDs like "ab", "cd", "ef"
...
function addApproval(approval: Approval): void {
  ...
  if (!exists) {
    ...
    const existingIds = approvals.approvals.map(a => a.id);
    approval.id = generateApprovalId(existingIds);
